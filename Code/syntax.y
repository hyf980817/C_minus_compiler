...
%token OP_STAR OP_DIV OP_MOD OP_ADD OP_SUB OP_SHL OP_SHR OP_BIT_AND OP_BIT_XOR OP_BIT_OR OP_GT OP_LT OP_GE OP_LE OP_EQ OP_NEQ  OP_AND OP_OR OP_ASSIGN
...
Expr : Expr OP_ASSIGN Expr     {$$ = initTreeNode(yytname[yyr1[yyn]], Expr); insertChild($$, 3, $1, $2, $3);}
    ...
    | Expr OP_AND Expr     {$$ = initTreeNode(yytname[yyr1[yyn]], Expr); insertChild($$, 3, $1, $2, $3);}
    | Expr OP_OR Expr     {$$ = initTreeNode(yytname[yyr1[yyn]], Expr); insertChild($$, 3, $1, $2, $3);}
    | OP_SUB Expr       {$$ = initTreeNode(yytname[yyr1[yyn]], Expr); insertChild($$, 2, $1, $2);}
